...
const ElevatorData = Array.apply(null, COUNTS).map((item, i) => ({id: i + 1, cur: 0, target: -1, counts: 0, direction: 0}))
const Task = {}
...
passengerObservable.pipe(filter(data => data.type === 0))
  .subscribe(({id, eid, target}) => {
    // 计算是否需要更新 target
    const item = ElevatorData.find(data => data.id === eid)
    ElevatorData.target = Math[item.direction > 0 ? 'max' : 'min'](target, ElevatorData.target)
    ElevatorData.counts++
    delete Task[id]
    elevatorSubject.next(getMsg(ElevatorData))
  })
